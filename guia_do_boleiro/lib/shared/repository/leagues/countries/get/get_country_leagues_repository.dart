...
abstract class GetCountryLeaguesRepository {
  Future<Either<Failure, List<League>>> getLeagues(
      String country, String season);
}

class GetCountryLeaguesRepositoryImpl extends GetCountryLeaguesRepository {
  final NetworkInfo networkInfo;
  final GetCountryLeaguesRemoteDataSource remoteDataSourceImpl;

  GetCountryLeaguesRepositoryImpl(
      {this.networkInfo, this.remoteDataSourceImpl});

  @override
  Future<Either<Failure, List<League>>> getLeagues(
      String country, String season) async {
    if (await networkInfo.isConnected) {
      try {
        final countryLeagues =
            await remoteDataSourceImpl.getCountryLeagues(country, season);
